...
class TimetablingSystem:
    ...
    def load_data(self, 
        ...
        try:
            ...
            if not self.data_ingestion.load_data(courses_file, instructors_file, rooms_file, groups_file):
                return False
    ...
    def _validate_data(self, skip_validation: bool = False) -> bool:
        ...
        try:
            ...
            if not is_valid:
                if skip_validation:
                    self.logger.warning("Data validation failed but continuing due to --skip-validation:")
                    for error in errors:
    ...
    def optimize(self, callback: Optional[Callable[[int, float, Dict[str, Any]], bool]] = None) -> Optional[Any]:
        """
        ...
        try:
            ...
            self.ga_engine.initialize(self.courses, self.instructors, self.rooms, self.groups)
    ...
    def generate_visualizations(self, 
        ...
        if not self.best_solution or not self.evolution_stats:
            self.logger.error("No solution available for visualization. Run optimize() first.")
            return False
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='GA-based University Course Timetabling System')
    
    # Input files
    parser.add_argument('--courses', default='data/sample_courses.csv', help='Path to courses CSV file (default: data/sample_courses.csv)')
    parser.add_argument('--instructors', default='data/sample_instructors.csv', help='Path to instructors CSV file (default: data/sample_instructors.csv)')
    parser.add_argument('--rooms', default='data/sample_rooms.csv', help='Path to rooms CSV file (default: data/sample_rooms.csv)')
    parser.add_argument('--groups', default='data/sample_groups.csv', help='Path to groups CSV file (default: data/sample_groups.csv)')
    
    # Output options
    parser.add_argument('--output', help='Output file path for schedule (default: timestamped directory)')
    parser.add_argument('--format', choices=['csv', 'excel', 'json'], default='csv', 
                       help='Output format (default: csv)')
    parser.add_argument('--detail', choices=['basic', 'detailed', 'summary'], default='detailed',
    ...
    parser.add_argument('--no-viz-individual', action='store_true',
                       help='Skip individual visualization plots (default: create them)')
    parser.add_argument('--no-viz-comprehensive', action='store_true',
                       help='Skip comprehensive visualization report (default: create it)')
    
    # GA parameters dynamically fetched from GA_CONFIG
    parser.add_argument('--population', type=int, default=GA_CONFIG['population_size'], 
                       help=f'Population size (default: {GA_CONFIG["population_size"]})')
    parser.add_argument('--generations', type=int, default=GA_CONFIG['max_generations'], 
                       help=f'Maximum generations (default: {GA_CONFIG["max_generations"]})')
    parser.add_argument('--mutation-rate', type=float, default=GA_CONFIG['mutation_rate'], 
                       help=f'Mutation rate (default: {GA_CONFIG["mutation_rate"]})')
    parser.add_argument('--crossover-rate', type=float, default=GA_CONFIG['crossover_rate'], 
                       help=f'Crossover rate (default: {GA_CONFIG["crossover_rate"]})')
    parser.add_argument('--elite-size', type=int, default=GA_CONFIG['elite_size'], 
                       help=f'Elite size (default: {GA_CONFIG["elite_size"]})')
    parser.add_argument('--tournament-size', type=int, default=GA_CONFIG['tournament_size'], 
                       help=f'Tournament size (default: {GA_CONFIG["tournament_size"]})')
    
    # Other options with defaults
    parser.add_argument('--print-schedule', action='store_true', default=True,
    ...
    try:
        ...
        # Load data
        if not system.load_data(args.courses, args.instructors, args.rooms, args.groups, args.skip_validation):
        ...
        if solution:
            ...
            if not args.no_visualizations:
                ...
                if system.generate_visualizations(
                    ...
                    print(f"📊 Visualizations saved to: {system.get_output_directory()}/visualizations/")
                else:
            ...
            if stats:
                ...
                print(f"  Best fitness: {stats.get('fitness', {}).get('best_achieved', 0):.6f}")
