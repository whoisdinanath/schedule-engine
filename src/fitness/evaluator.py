...
class FitnessEvaluator:
    ...
    def _calculate_fitness(
        self, hard_violations: Dict[str, int], soft_penalties: Dict[str, float]
    ) -> float:
        """
        ...
        if self.adaptive_weights:
            ...
            fitness = -(
                self.alpha * total_hard_violations + self.beta * total_soft_penalty
            )
    ...
    def calculate_fitness_diversity_bonus(
        self, chromosome: Chromosome, population_chromosomes: List[Chromosome]
    ) -> float:
        """
    ...
    def _calculate_chromosome_distance(
        self, chr1: Chromosome, chr2: Chromosome
    ) -> float:
        """
    ...
    def compare_chromosomes(
        ...
        """
        breakdown1 = self.get_fitness_breakdown(
            chr1, courses, instructors, rooms, groups
        )
        breakdown2 = self.get_fitness_breakdown(
            chr2, courses, instructors, rooms, groups
        )
        ...
        return {
            ...
            "better_chromosome": (
                1 if breakdown1["fitness"] > breakdown2["fitness"] else 2
            ),
            "distance": self._calculate_chromosome_distance(chr1, chr2),
        }

    def update_weights(
        self, alpha: float, beta: float, diversity_bonus: float = None
    ) -> None:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        """
        avg_time = (
            self.total_evaluation_time / self.evaluation_count
