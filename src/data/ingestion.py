...
class DataIngestion:
    ...
    def _load_instructors(self, file_path: str) -> bool:
        ...
        try:
            ...
            if file_path.endswith('.json'):
                data = self._load_json(file_path)
                instructors_data = data if isinstance(data, list) else data.get('instructors', [])
            else:
                df = pd.read_csv(file_path)
    ...
    def _create_course_from_data(self, data: Dict[str, Any]) -> Optional[Course]:
        ...
        try:
            ...
            qualified_instructors = self._parse_list_field(data.get('qualified_instructor_ids', ''))
            ...
            course = Course(
                ...
                room_consistency_required=bool(data.get('room_consistency_required', False))
            )
    ...
    def _create_instructor_from_data(self, data: Dict[str, Any]) -> Optional[Instructor]:
        """Create Instructor object from data dictionary."""
        try:
            # Parse qualified courses
            qualified_courses = self._parse_list_field(data.get('qualified_courses', ''))
    ...
    def _create_room_from_data(self, data: Dict[str, Any]) -> Optional[Room]:
        ...
        try:
            ...
            room = Room(
                ...
                room_type=str(data['type']),  # Map 'type' from CSV to 'room_type' parameter
                available_slots=available_slots,
                equipment=equipment,
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_sessions_per_week': sum(course.sessions_per_week for course in self.courses.values()),
            'total_students': sum(group.student_count for group in self.groups.values())
        }
