...
from src.utils.config import TIME_SLOTS
...
class EvolutionVisualizer:
    ...
    def plot_fitness_evolution(
        ...
        ax.fill_between(
            generations, best_fitness, worst_fitness, alpha=0.2, color="gray"
        )
    ...
    def plot_constraint_violations(
        ...
        ax1.plot(
            generations, hard_violations, "r-", linewidth=2, marker="o", markersize=3
        )
        ...
        ax1.set_title(
            "Hard Constraint Violations Over Generations",
            fontsize=14,
            fontweight="bold",
        )
        ...
        ax2.set_title(
            "Soft Constraint Violations Over Generations",
            fontsize=14,
            fontweight="bold",
        )
    ...
    def plot_diversity_evolution(
        ...
        ax.set_title(
            "Population Diversity Over Generations", fontsize=14, fontweight="bold"
        )
        ax.grid(True, alpha=0.3)
    ...
    def plot_schedule_heatmap(
        ...
        for gene in chromosome:
            try:
                ...
                # Handle quanta-based scheduling
                for quantum in gene.quanta:
                    # Assuming you have a method to convert quantum to day/time
                    # You'll need to implement this or use your QuantumTimeSystem
                    # For now, using a placeholder
                    day_idx = quantum % len(days)  # Simplified mapping
                    slot_idx = (quantum // len(days)) % len(time_slots)
                    time_idx = day_idx * len(time_slots) + slot_idx

                    if time_idx < schedule_matrix.shape[1]:
    ...
    def plot_schedule_heatmap_with_qts(
        self,
        chromosome: List[SessionGene],
        ...
        """
        Create a heatmap visualization of the schedule with proper quantum time conversion.

        Args:
            chromosome: List of SessionGene representing the schedule
            ...
            show_plot: Whether to display the plot
        """
        # Create schedule matrix based on actual quantum time system
        days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
        time_slots = [f"{hour:02d}:00" for hour in range(8, 21)]  # 8 AM to 8 PM

        # Determine entities based on view type
        if view_type == "room":
            entities = list(rooms.keys())
            title = "Room Utilization Schedule"
        elif view_type == "instructor":
        ...
        schedule_matrix = np.zeros((len(entities), len(days) * len(time_slots)))
        entity_labels = [
            entities[i][:10] + "..." if len(entities[i]) > 10 else entities[i]
            for i in range(len(entities))
        ]
        time_labels = [f"{day[:3]}\\n{slot}" for day in days for slot in time_slots]

        # Fill matrix using quantum time system
        for gene in chromosome:
            try:
                if view_type == "room" and gene.room_id in entities:
                    entity_idx = entities.index(gene.room_id)
                ...
                # Convert each quantum to day and time using QTS
                for quantum in gene.quanta:
                    try:
                        day, time_str = qts.quanta_to_time(quantum)
                        day_idx = days.index(day)

                        # Extract hour from time string (format: "HH:MM")
                        hour = int(time_str.split(":")[0])
                        if 8 <= hour <= 20:  # Within our time range
                            slot_idx = hour - 8
                            time_idx = day_idx * len(time_slots) + slot_idx
                        ...
                        # Handle cases where quantum conversion fails
                        continue
    ...
    def plot_workload_distribution(
        ...
        for gene in chromosome:
            if gene.instructor_id in instructors and gene.course_id in courses:
                ...
                # Count the number of quanta for this gene and convert to hours
                # Assuming each quantum represents a fixed time unit
                workloads[gene.instructor_id] += (
                    len(gene.quanta) * 0.5
                )  # 0.5 hours per quantum
        ...
        bars = ax.bar(
            instructor_names, hours, color=plt.cm.viridis(np.linspace(0, 1, len(hours)))
        )
    ...
    def create_comprehensive_report(
        ...
        violation_history = evolution_stats.get("violations", {}).get(
            "violation_history", []
        )
        ...
        diversity_history = evolution_stats.get("fitness", {}).get(
            "diversity_history", []
        )
